...
from typing import Any, Awaitable, Callable, List, Optional, Mapping, MutableMapping, cast
...
class BidiSession:
    ...
    def __init__(self,
        ...
        if parsed_url.path == "/":
            if session_id is None:
                ...
                websocket_url = urljoin(websocket_url, "session/%s" % session_id)
        else:
            if session_id is not None:
                if parsed_url.path != "/session/%s" % session_id:
                    raise ValueError("WebSocket URL %s doesn't match session id" % session_id)
            else:
                if parsed_url.path != "/session":
                    raise ValueError("WebSocket URL %s doesn't match session url" % session_id)
        ...
        self.requested_capabilites = requested_capabilities
        self.capabilities = capabilities
        self.session_id = session_id

        self.command_id = 0
        self.pending_commands: MutableMapping[int, "asyncio.Future[Any]"] = {}
        self.event_listeners: MutableMapping[Optional[str], List[Callable[[Mapping[str, Any]], Any]]] = defaultdict(list)
    ...
    async def on_message(self, data: Mapping[str, Any]) -> Any:
    ...
    def add_event_listener(self,
                           name: Optional[str],
                           fn: Callable[[Mapping[str, Any]], Awaitable[Any]]) -> None:
        ...
        :param fn: Async callback function that recieves ecent data
        """
...
class Transport:
    ...
    def __init__(self, url: str,
                 msg_handler: Callable[[Mapping[str, Any]], "asyncio.Future[Any]"],
                 loop: Optional[asyncio.AbstractEventLoop] = None):
        self.url = url
    ...
    async def handle(self, msg: str) -> Any:
        logger.debug("← %s", msg)
        data = json.loads(msg)
...
class Session(BidiModule):
    ...
    @new.result
    def _new(self, result: Mapping[str, Any]) -> Mapping[str, Any]:
        caps = result["capabilities"]
        capabilities = cast(Mapping[str, Any], caps)
        return capabilities
