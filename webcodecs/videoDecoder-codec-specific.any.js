...
promise_test(async t => {
  ...
  assert_object_equals(
      support.config.visibleRegion, config.visibleRegion, 'visibleRegion');
  assert_equals(
      support.config.displayWidth, config.displayWidth, 'displayWidth');
  assert_equals(
      support.config.displayHeight, config.displayHeight, 'displayHeight');
  assert_false(
      support.config.hasOwnProperty('futureConfigFeature'),
      'futureConfigFeature');
...
promise_test(async t => {
  ...
  await test(
      t, {...CONFIG, visibleRegion: {...CONFIG.visibleRegion, left: 1}},
      'out of bounds visibleRegion');
...
promise_test(async t => {
  ...
  callbacks.output = frame => {
    outputs++;
    assert_object_equals(
        frame.visibleRegion, CONFIG.visibleRegion, 'visibleRegion');
